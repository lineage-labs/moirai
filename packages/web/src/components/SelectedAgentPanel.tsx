...
export function SelectedAgentPanel() {
  ...
  const [marketStates, setMarketStates] = useState<
    Record<string, "idle" | "pending" | "listed" | "error">
  >({});
  ...
  const lastMarketEvent = selectedAgentId
    ? events
        .filter(
          (e) =>
            (e.kind === "AGENT_LISTED" ||
              e.kind === "AGENT_DELISTED" ||
              e.kind === "MARKETPLACE_ERROR") &&
            e.actorId === selectedAgentId,
        )
        .at(-1)
    : undefined;
  ...
  useEffect(() => {
    ...
    if (lastMarketEvent.kind === "AGENT_LISTED") {
      ...
      addToast(
        `Agent ${
          agents[selectedAgentId]?.name ?? getPersonality(selectedAgentId).name
      ...
      const msg =
        (lastMarketEvent.payload?.message as string) ?? "Request failed";
  ...
  const chainScanBase = import.meta.env["VITE_CHAINSCAN_BASE_URL"] as
    | string
    | undefined;
  const chainScanUrl =
    tokenId && nftContractAddress && chainScanBase
  ...
  function handleList() {
    ...
    sendWs({
      ...
      agentId,
      tokenId,
      salePriceWei: ogToWei(listPrice),
    });
  }
  ...
  const stableId = `AG-${String(hashAgent(agentId) * 17)
    .padStart(4, "0")
    .slice(0, 4)}`;
  ...
  return (
    <aside
      style={{
        ...
        padding: 14,
        background:
          "linear-gradient(180deg, rgba(28, 22, 15, 0.86), rgba(18, 15, 12, 0.82))",
        border: "1px solid rgba(231, 190, 110, 0.22)",
        boxShadow:
          "0 24px 60px rgba(0,0,0,0.46), inset 0 1px 0 rgba(255,235,180,0.08)",
    ...
    >
      ...
      >
        <div style={{ position: "relative", flexShrink: 0 }}>
          <img
            src={agent.image ?? getAgentAvatar(agentId)}
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: "#f4dfb2" }}>
            {agent.name ?? personality.name}
          </div>
          <div style={{ color: "#bda16f", fontSize: 10 }}>
            AGE {18 + (hashAgent(agentId) % 14)}
          </div>
        ...
        {chainScanUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            title="View NFT on 0G Chainscan"
            style={{
              display: "flex",
          ...
          >
            ...
            >
              ...
              />
              <path
                d="M2 9h20M8 2l-6 7M16 2l6 7M12 22L2 9M12 22L22 9"
                stroke="#7fb3d3"
                strokeWidth="1.2"
      ...
      <Section title="Status">
        <div
          style={{
            ...
            alignItems: "center",
            gap: 6,
            color: targeted ? "#ff7b62" : "#9bd76e",
            fontSize: 12,
            fontWeight: 700,
          }}
        >
          <span>
            {targeted ? "🏃" : agent.status === "reasoning" ? "🤔" : "🧘"}
          </span>
        ...
        >
          <div
            style={{
              ...
              height: "100%",
              background: health > 0.4 ? "#53c66d" : "#e45a45",
              borderRadius: 999,
              boxShadow: `0 0 10px ${health > 0.4 ? "#53c66d" : "#e45a45"}`,
            }}
          />
        </div>
      </div>
      ...
      <Section title="Goal">
        <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
          ...
          <div>
            ...
            >
              {targeted
                ? "Get help to escape the lion"
                : "Gather useful skills"}
            </div>
          </div>
        </div>
      </Section>
      <Section title="Traits">
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {(agent.traits ?? personality.traits ?? []).map((trait) => (
      ...
      <Section title="Skills">
        <div style={{ display: "grid", gap: 6 }}>
          {agent.knownSkillIds.length === 0 ? (
            <span style={{ color: "#8d7b63", fontSize: 11 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <div
                style={{
                  ...
                  padding: "5px 8px",
                  borderRadius: 6,
                  background: "rgba(143,209,109,0.10)",
                  border: "1px solid rgba(143,209,109,0.22)",
                }}
              >
                <span style={{ color: "#8fd16d", fontSize: 10 }}>✓</span>
                <span
                  style={{ color: "#a9c882", fontSize: 10, fontWeight: 700 }}
                >
                  Listed for sale
                </span>
              </div>
              <button
                ...
                disabled={marketStates[agentId] === "pending"}
                style={btnStyle("#e45a45", "#ff8c74")}
              >
                {marketStates[agentId] === "pending"
                  ? "Delisting…"
                  : "Delist Agent"}
              </button>
            </div>
