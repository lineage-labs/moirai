  ...
  return (
    <aside
      style={{
        ...
        padding: 12,
        background:
          "linear-gradient(180deg, rgba(28, 22, 15, 0.84), rgba(18, 15, 12, 0.80))",
        border: "1px solid rgba(231, 190, 110, 0.18)",
        boxShadow:
          "0 22px 58px rgba(0,0,0,0.44), inset 0 1px 0 rgba(255,235,180,0.08)",
      ...
      {active.length === 0 ? (
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {active.map((crisis) => {
            ...
            const eta = Math.max(
              0,
              crisis.startedAtTick + crisis.deadlineTicks - tick,
            );
            const isLion = type === "lion";
            const etaSeconds = Math.max(
              0,
              Math.ceil((30000 - (nowMs - crisis.startedAtMs)) / 1000),
            );
            return (
              <div
                key={crisis.id}
                  ...
                  >
                    {lionIcon && (
                      <img
                        ...
                        style={{
                          filter:
                            "brightness(0) saturate(100%) invert(73%) sepia(47%) saturate(592%) hue-rotate(341deg) brightness(94%) contrast(92%)",
                        }}
                      />
                    )}
                  </div>
                    ...
                    {crisis.targets.map((target) => (
                      <span
                        ...
                        style={{
                          ...
                          background: isLion
                            ? "rgba(210,60,44,0.22)"
                            : "rgba(210,160,44,0.20)",
                          border: isLion
                            ? "1px solid rgba(255,113,90,0.22)"
                            : "1px solid rgba(230,190,80,0.28)",
                        }}
                      >
                        {target}
                      </span>
