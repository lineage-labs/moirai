...
describe("evolve", () => {
  it("accepts a candidate when self-eval score >= threshold", async () => {
    ...
    const events: Event[] = [];
    const kernel = await createKernel({ compute, storage, network, emit: (e) => events.push(e) });

    const result = await kernel.evolve({ personality, environment, crisis, context, knownSkills: [] });
  ...
  it("rejects a candidate when self-eval score < threshold", async () => {
    ...
    expect(events.map((e) => e.kind)).toContain("SKILL_REJECTED");
    expect(events.map((e) => e.kind)).not.toContain("SKILL_ACCEPTED");
  });
  ...
  it("lowers threshold for higher-risk personality (formula 0.6 - (risk - 0.5)*0.4)", async () => {
    ...
    const result = await kernel.evolve({ personality: reckless, environment, crisis, context, knownSkills: [] });
    expect(result.status).toBe("accepted");
  });
  ...
  it("throws EvolveError when reasoning receipt is not verifiable", async () => {
    ...
    await expect(
      kernel.evolve({ personality, environment, crisis, context, knownSkills: [] }),
